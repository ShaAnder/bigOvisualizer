...
export function computeOps(notation: Notation, n: number) {
  ...
  if (notation === 'O(log n)') return Math.ceil(Math.log2(Math.max(1, Math.floor(n)) + 1));
...
export function computeTotalSeconds(ops: number, perOp: number, perOpUnit: 'ns'|'us'|'ms'|'s') {
...
export function computeTotalBytes(metric: Metric, notation: Notation, n: number, bytesPerItem: number) {
  ...
  if (notation === 'O(n)' || notation === 'O(n log n)') return Math.max(0, n) * bytesPerItem; // e.g., filter/merge buffers
