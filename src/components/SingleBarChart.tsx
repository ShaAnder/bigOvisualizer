...
import { formatBytes, formatDuration, toSeconds, widthFromBytes, widthFromSeconds } from '../utils/format';
...
export function SingleBarChart({
  ...
  const ops = useMemo(() => {
    ...
    if (notation === 'O(n log n)') return Math.max(0, n) * Math.max(1, Math.ceil(Math.log2(Math.max(1, n))));
    // O(n^2)
  ...
  const totalSeconds = useMemo(() => {
    if (typeof overrideSeconds === 'number' && overrideSeconds >= 0) return overrideSeconds;
  ...
  const totalBytes = useMemo(() => {
    if (typeof overrideBytes === 'number' && overrideBytes >= 0) return overrideBytes;
  ...
  const width = metric === 'time' ? widthFromSeconds(totalSeconds) : widthFromBytes(totalBytes);
  ...
  const iterations = useMemo(() => {
    ...
    if (notation === 'O(1)') return 1;
    if (notation === 'O(log n)') return Math.max(1, Math.ceil(Math.log2(Math.max(1, nn) + 1)));
    if (notation === 'O(n)') return nn;
    if (notation === 'O(n log n)') return nn * Math.max(1, Math.ceil(Math.log2(Math.max(1, nn))));
  ...
  const detailHint = useMemo(() => {
    ...
    if (notation === 'O(log n)') return `Decisions: ceil(log2(n + 1)) = ${iterations.toLocaleString()}`;
    if (notation === 'O(n)') return `Iterations: n = ${nn.toLocaleString()}; approx ops ≈ c x n (checks + i++ + body).`;
  ...
  return (
    <div className="chart">
      <div className="metrics">
        {metric === 'time' ? (
          <>
            <div>{`~${ops.toLocaleString()} operations -> ${formatDuration(displaySeconds)} (seconds)`}</div>
  ...
  {detailHint ? <div className="hint" style={{ marginTop: 6 }}>{detailHint}</div> : null}
