...
export const TABS: TabSpec[] = [
  ...
  detail: 'Constant time means the number of steps does not change with n. A lookup by index or a hash map get are classic O(1) operations.',
  expected: 'Time stays flat as n grows; extra space remains constant (input storage is not counted).',
  ...
  realWorld: 'Accessing an array element by index or checking a Set for membership.',
  ...
  title: 'O(log n) - Logarithmic',
    description: 'Each step cuts the problem down (divide-and-conquer). Doubling n adds ~one step.',
  detail: 'Logarithmic time arises when the input is halved (or reduced by a constant factor) repeatedly; doubling n adds ~one step.',
    expected: 'Time grows slowly with n; space is often constant aside from the data.',
  ...
  realWorld: 'Binary searching a sorted phone book or looking up in a balanced BST.',
...
function binarySearch(arr, target) { /* O(1) extra space */ }`,
  ...
  detail: 'Linear time algorithms touch each item a constant number of times; work scales directly with n.',
    expected: 'Time doubles as n doubles; space depends on what you store (often O(1) or O(n)).',
  ...
  title: 'O(n log n) - Linearithmic',
    description: 'Do O(log n) work for each of the n items (e.g., balanced splits).',
  detail: 'Divide-and-conquer that does near-linear work across log n levels of recursion; often seen in efficient sorts.',
    expected: 'Time grows faster than linear but much slower than quadratic; space varies (e.g., mergesort uses O(n)).',
...
function mergesort(arr) {
  ...
  detail: 'For each item, you loop over all items again, leading to n x n operations; avoid when n grows large.',
    expected: 'Time grows quickly; space may be O(1) unless building n x n structures.',
